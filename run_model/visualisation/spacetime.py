...
class ST_GridPlot(object):
    def __init__(self, columns, col, fig, ax, train_df, test_df, cax_on_right, norm_on_training=True, label='', geopandas_flag=False):
...
class ST_SliderPlot(object):
    ...
    def set_text_format(self):
        ...
        self.slider.valtext.set_text(datetime.fromtimestamp(self.slider.val).strftime("%Y-%m-%d %H"))
...
class ST_ScatterPlot(object):
    def __init__(self, columns, fig, ax, grid_plot, grid_plot_flag, callback, train_df):
        ...
        if grid_plot_flag:
            ...
            #self.norm = matplotlib.colors.Normalize(vmin=np.min(self.train_df[self.columns['pred']]),vmax=np.max(self.train_df[self.columns['pred']]))
            self.norm = matplotlib.colors.Normalize(vmin=np.min(self.train_df[self.columns['pred']]),vmax=1000)
    ...
    def get_closest_observed(self, p):
        d = np.array(self.train_df[[self.columns['x'], self.columns['y']]]).astype(np.float32)
...
class SpaceTimeVisualise(object):
    def __init__(self, train_df, test_df, geopandas_flag=True):
        ...
        self.start_epoch = self.unique_epochs[0]
    ...
    def show(self):
        ...
        self.grid_plot_1_ax = self.fig.add_subplot(self.gs[0:7, 0:2]) # first row, first col
        self.grid_plot_2_ax = self.fig.add_subplot(self.gs[0:7, 2:4]) # first row, second col
        self.epoch_slider_ax = self.fig.add_subplot(self.gs[7, 1:3]) # first row, second col
        self.time_series_ax = self.fig.add_subplot(self.gs[8:11, :]) # full second row
        self.scale_slider_ax = self.fig.add_subplot(self.gs[11, 1:3]) # first row, second col

        if self.grid_plot_flag:
            self.val_grid_plot = ST_GridPlot(self.columns, 'pred', self.fig, self.grid_plot_1_ax, self.train_df, self.test_df, cax_on_right=False, norm_on_training=True, label='NO2', geopandas_flag=self.geopandas_flag)
            self.val_grid_plot.setup()

            self.var_grid_plot = ST_GridPlot(self.columns, 'var', self.fig, self.grid_plot_2_ax, self.train_df, self.test_df, cax_on_right=False, norm_on_training=True, label='NO2', geopandas_flag=self.geopandas_flag)
            self.var_grid_plot.setup()
        else:
            self.val_grid_plot = None
            self.var_grid_plot = None

        self.val_scatter_plot = ST_ScatterPlot(self.columns, self.fig, self.grid_plot_1_ax, self.val_grid_plot, self.grid_plot_flag, self.update_timeseries, self.train_df)
        self.val_scatter_plot.setup()

        self.slider_plot = ST_SliderPlot(self.fig, self.epoch_slider_ax, self.unique_epochs, self.update_epoch)
        self.slider_plot.setup(self.start_epoch)

        self.time_series_plot = ST_TimeSeriesPlot(self.columns, self.fig, self.time_series_ax, self.train_df, self.test_df, self.test_start, self.grid_plot_flag)
