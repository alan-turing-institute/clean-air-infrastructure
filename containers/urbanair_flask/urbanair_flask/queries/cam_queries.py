    ...
    def query(self, session, id):
        ...
        for date in list(dates):
            for type in counts:
                ...
                for row in rows:
                    if (
                        row["creation_datetime"] == date
                        and type == row["detection_class"]
                    ):
                        counts[type][-1] = row[
                            "counts"
...
class CamsSnapshot(APIQueryMixin):
    def query(self, session, type_):
        ...
        try:
            if type_ == "people":
                res = session.execute(
                    "select * from jamcam.sum_counts_last_hour_people"
                )
            elif type_ == "cars":
                ...
                res = session.execute(
                    "select * from jamcam.sum_counts_last_hour_trucks"
                )
            elif type_ == "motorbikes":
                res = session.execute(
                    "select * from jamcam.sum_counts_last_hour_motorbikes"
                )
            else:
