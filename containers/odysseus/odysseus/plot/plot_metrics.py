...
def plot_comparison_to_baseline(
    baseline_df: pd.DataFrame, comparison_df: pd.DataFrame
) -> go.Figure:
    """
    ...
    data = [
        ...
        go.Scatter(
            x=baseline_df.hour,
            y=baseline_df.n_vehicles_in_interval,
            mode="markers",
            name="baseline",
        ...
        go.Scatter(
            ...
            y=comparison_df.n_vehicles_in_interval,
            mode="markers",
            name="comparison",
        ),
    ]
    layout = dict(xaxis_title="Hour", yaxis_title="Number of vehicles")
    fig = go.Figure(data=data, layout=layout)

    y_height = (
        baseline_df.n_vehicles_in_interval.mean()
        + 3 * baseline_df.n_vehicles_in_interval.std()
    )
    fig.add_shape(
        # Line Vertical
        dict(
