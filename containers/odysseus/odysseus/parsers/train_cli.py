...
def scoot(
    ...
    if kernel == "periodic":
        kernel_params = PeriodicKernelParams(
            ...
            base_kernel=KernelParams(  # NOTE default base is RBF
                name="rbf", lengthscales=lengthscales, variance=variance,
            ),
            period=period,
        )
    else:
        kernel_params = KernelParams(
            name=kernel, lengthscales=lengthscales, variance=variance,
        )
    ...
    if model_name == ModelName.svgp:
        ...
        model_params = ScootModelParams(
            kernel=kernel_params, maxiter=maxiter, optimizer=optimizer,
        )
    # load the datasets
    logger.info(
        "Get scoot training data from %s to %s for %s detectors.",
        train_start,
        train_upto,
        limit,
    )
    ...
    logger.info(
        "Writing experiment settings and parameters to the scoot modelling instance tables."
    )
    experiment.update_remote_tables()
    logger.info(
        "Training %s Sparse Variational GP models with a %s kernel.",
        len(datasets),
        kernel,
    )
