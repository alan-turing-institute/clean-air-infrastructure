...
def test_add_instance_to_experiment(simple_experiment, simple_instance):
    ...
    assert simple_experiment.get_file_manager(simple_instance.instance_id).input_dir.exists()
...
def test_add_training_dataset(simple_setup_experiment, simple_instance):
    ...
    assert simple_setup_experiment.get_training_dataset(simple_instance.data_id) == dataset
...
def test_load_datasets(simple_setup_experiment, different_instance, simple_instance):
    ...
    assert simple_setup_experiment.get_training_dataset(different_instance.data_id).shape[0] == simple_setup_experiment.num_ones
    assert simple_setup_experiment.get_training_dataset(simple_instance.data_id).shape[0] == simple_setup_experiment.num_ones
    assert simple_setup_experiment.get_test_dataset(different_instance.data_id).shape[0] == simple_setup_experiment.num_ones
    assert simple_setup_experiment.get_test_dataset(simple_instance.data_id).shape[0] == simple_setup_experiment.num_ones
