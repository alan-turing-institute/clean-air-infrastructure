...
def training_df() -> pd.DataFrame:
    ...
    data_df = pd.DataFrame(dict(
        measurement_start_utc=timerange,
        NO2=np.random.rand(24),
    ))
...
def model_data(
    ...
    monkeypatch.setattr(ModelData, "_ModelData__validate_config", mock.mock_validate_config)
    monkeypatch.setattr(ModelData, "_ModelData__generate_full_config", mock.mock_generate_full_config)
    monkeypatch.setattr(ModelData, "get_training_data_inputs", mock.mock_get_training_data_inputs)
    monkeypatch.setattr(ModelData, "get_pred_data_inputs", mock.mock_get_pred_data_inputs)
    dataset = ModelData(no_features_data_config, secretfile=secretfile, connection=connection)
