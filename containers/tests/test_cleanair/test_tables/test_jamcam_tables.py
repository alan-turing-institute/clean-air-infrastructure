...
def test_today_stats_view(secretfile, connection):
	...
	db_instance.commit_records(
		[
			JamCamDayStats(id=1, camera_id="test", date=today, detection_class='car', count=10,source=1),
			JamCamDayStats(id=2, camera_id="test", date=yesterday, detection_class='bus', count=20, source=1)
		],
		on_conflict="ignore",
