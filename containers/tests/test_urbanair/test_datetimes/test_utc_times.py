...
class TestUTCTime:
    ...
    def test_utctime_is_good(self):
        ...
        utc_string = json.loads(UTCTime(measurement_start_utc=self.utc_time_string_good).json())['measurement_start_utc']
        assert utc_string == self.utc_time_string_good

    def test_bad_utctime_is_fixed(self):
        """Test a good datetime stamp is handled correctly"""
        utc_string = json.loads(UTCTime(measurement_start_utc=self.utc_time_string_bad).json())['measurement_start_utc']
