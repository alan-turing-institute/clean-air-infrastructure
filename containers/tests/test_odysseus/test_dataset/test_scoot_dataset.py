...
def validate_scoot_dataset(dataset: ScootDataset) -> None:
    ...
    nhours = (as_datetime(dataset.data_config.upto) - as_datetime(dataset.data_config.start)).days * 24
    ndetectors = len(dataset.data_config.detectors)
    assert nhours * ndetectors == len(dataset.dataframe)

def test_scoot_dataset_init(secretfile: str, connection: Connector, scoot_config: ScootConfig, scoot_preprocessing: ScootPreprocessing) -> None:
    ...
    dataset = ScootDataset(scoot_config, scoot_preprocessing, secretfile=secretfile, connection=connection)
