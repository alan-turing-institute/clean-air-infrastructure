...
def main():
    ...
    if (
        args.command == "batch"
        and isinstance(args.batch_start, int)
        and isinstance(args.batch_size, int)
    ):
    ...
    data_df = instance_query.get_data_config(
        detectors=detectors, baseline_period=args.baseline_period, output_type="df",
    )
    ...
    instance_df = instance_query.get_instances_with_params(
        tag=args.tag, data_ids=data_df["data_id"].to_list(), output_type="df"
    )
    # load models from file
    model_dict = load_models_from_file(
        instance_df["instance_id"], os.path.join(args.root, args.experiment, "models")
    )
    ...
    traffic_metric.batch_evaluate_model(
        instance_df["instance_id"], model_list, datasets
    )
