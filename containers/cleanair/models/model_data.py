...
class ModelData(DBWriter, DBQueryMixin):
    ...
    def __validate_config(self, config):
        ...
        if isinstance(train_interest_points, list):
            self.__check_ipoints_available(train_interest_points, train_sources)

        pred_interest_points = config["pred_interest_points"]
        if isinstance(pred_interest_points, list):
            self.__check_ipoints_available(pred_interest_points, pred_sources)
        ...
        if config["include_satellite"]:
            ...
            if isinstance(satellite_interest_points, list):
                self.__check_ipoints_available(pred_interest_points, ["satellite"])
    ...
    def __check_ipoints_available(self, interest_points, sources):
    ...
    def __select_features(
        ...
        with self.dbcnxn.open_session() as session:
            ...
            interest_point_query = self.get_available_interest_points(sources).filter(
