...
class DBQueryMixin:
    ...
    def get_available_interest_points(self, sources, point_ids=None):
        ...
        with self.dbcnxn.open_session() as session:
            ...
            aqe_sources_q = (
                session.query(
                    *base_query_columns, func.min(AQESite.date_opened), func.min(AQESite.date_closed)
                )
                .join(AQESite, isouter=True).group_by(*base_query_columns)
                .filter(
            ...
            laqn_sources_q = (
                session.query(
                    *base_query_columns, func.min(LAQNSite.date_opened), func.min(LAQNSite.date_closed)
                )
                .join(LAQNSite, isouter=True).group_by(*base_query_columns)
                .filter(
