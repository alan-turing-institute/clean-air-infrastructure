...
class DBQueryMixin():
    ...
    def get_available_dynamic_features(self, start_date, end_date):
        ...
        with self.dbcnxn.open_session() as session:
            ...
            available_dynamic_sq = session.query(
                IntersectionValueDynamic.feature_name,
                func.min(IntersectionValueDynamic.measurement_start_utc).label(
                    'min_date'),
                func.max(IntersectionValueDynamic.measurement_start_utc).label('max_date')).group_by(
                    IntersectionValueDynamic.feature_name).subquery()

            available_dynamic_q = session.query(available_dynamic_sq).filter(
                and_(available_dynamic_sq.c['min_date'] <= start_date,
    ...
    def get_available_interest_points(self, sources):
        ...
        with self.dbcnxn.open_session() as session:
            ...
            remaining_sources_q = session.query(*base_query_columns,
                ...
                MetaPoint.source.in_([source for source in sources if source not in [
                    'laqn', 'aqe', 'satellite']]),
            ...
            aqe_sources_q = session.query(*base_query_columns,
                                          AQESite.date_opened,
                                          AQESite.date_closed
                                          ).join(AQESite, isouter=True).filter(
            ...
            laqn_sources_q = session.query(*base_query_columns,
                                           ...
                                           ).join(LAQNSite, isouter=True).filter(
            ...
            all_sources_q = session.query(all_sources_sq).filter(all_sources_sq.c.date_closed.is_(None))
