...
def total_num_features(data_config: DataConfig) -> int:
    ...
    num_static_features = len(data_config.static_features) * len(data_config.buffer_sizes)
    num_dynamic_features = len(data_config.dynamic_features) * len(data_config.buffer_sizes)
    return (
        num_static_features
