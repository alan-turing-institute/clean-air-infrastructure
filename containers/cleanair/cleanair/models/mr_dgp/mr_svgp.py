...
class MR_SVGP(Parameterized):
    ...
    def marginal(self, m, s_chol, k_zz, k_xz, k_xx):
        ...
        _s_chol = s_chol
        s_chol = tf.tile(
            tf.expand_dims(s_chol, 0), [tf.shape(k_xz)[0], 1, 1]
        )  # N x M x M
        ...
        if self.white:
            mu = tf.matmul(
                ...
                tf.linalg.triangular_solve(
                    tf.transpose(k_zz_chol, [0, 2, 1]), m, lower=False, name="m_"
                ),
            )
        else:
            mu = tf.matmul(
                k_xz,
                tf.linalg.triangular_solve(
                    tf.transpose(k_zz_chol),
                    tf.linalg.triangular_solve(k_zz_chol, m, lower=True),
                    lower=False,
                ),
            )

        A = tf.linalg.triangular_solve(
            k_zz_chol, tf.transpose(k_xz, [0, 2, 1]), lower=True, name="A1_"
        )  # N x M x S
