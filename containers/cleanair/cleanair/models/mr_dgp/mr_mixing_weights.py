...
class MR_Average_Mixture(MR_Mixing_Weights):
    ...
    def predict(self, base_mu, base_sig, dgp_mu, dgp_sig, num_samples=1):
        ...
        sig = ((1 / (1 + len(dgp_sig))) ** 2) * tf.reduce_sum(
            [base_sig_samples] + dgp_sig, axis=0
        )
        return mu, sig
    ...
    def sample(self, base_mu, base_sig, dgp_mu, dgp_sig, num_samples):
        ...
        samples = utils.reparameterize(
            base_mu_samples, base_sig_samples, z, full_cov=False
...
class MR_Base_Only(MR_Mixing_Weights):
    ...
    def predict(self, base_mu, base_sig, dgp_mu, dgp_sig, num_samples=1):
        base_mu_samples = tf.tile(
            tf.expand_dims(base_mu[self.i], 0), [num_samples, 1, 1, 1]
        )
        base_sig_samples = tf.tile(
            tf.expand_dims(base_sig[self.i], 0), [num_samples, 1, 1, 1]
        )
...
class MR_Variance_Mixing(MR_Mixing_Weights):
    ...
    def predict(self, base_mu, base_sig, dgp_mu, dgp_sig, num_samples=1):
        ...
        for i in range(len(base_sig)):
            ...
            pred_var = tf.tile(
                tf.expand_dims(pred_var, 0), [tf.shape(dgp_mu[0])[0], 1, 1, 1]
            )
        ...
        sig_total = tf.multiply(
            1 - pred_vars[0], tf.multiply(1 - pred_vars[0], base_mu_samples)
        )
        ...
        for i in range(len(dgp_mu)):
            if reset_prev_mu:
                ...
                prev_sig = tf.multiply(
                    1 - pred_vars[i], tf.multiply(1 - pred_vars[i], prev_mu)
                )

                mu_weight = tf.multiply(pred_vars[i], mu_weight)
                sig_weight = tf.multiply(
                    pred_vars[i], tf.multiply(pred_vars[i], sig_weight)
                )
...
class MR_Variance_Mixing_1(MR_Mixing_Weights):
    ...
    def predict(self, base_mu, base_sig, dgp_mu, dgp_sig, num_samples=1):
        ...
        mu_total = tf.multiply(1 - pred_vars[0], base_mu_samples)
        sig_total = tf.multiply(
            1 - pred_vars[0], tf.multiply(1 - pred_vars[0], base_sig_samples)
        )
