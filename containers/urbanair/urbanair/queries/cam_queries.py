...
class CamRecent(APIQueryMixin):
    ...
    def query(self, session, id):
        ...
        if len(idparts[0]) != 5 or len(idparts[1]) != 5 or not idparts[0].isnumeric() or not idparts[1].isnumeric():
            return None
        res = session.execute(
            "select counts, detection_class, creation_datetime from video_stats where camera_id = '" + id + "' and video_upload_datetime > (select max(video_upload_datetime) from video_stats) - interval '12 hours';")
