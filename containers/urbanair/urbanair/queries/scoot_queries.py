...
class ScootPercAvailability(APIQueryMixin):
    ...
    @db_query
    def query(self, session, baseline, start_time=None, end_time=None, only_missing=False):
        """Check what data is available between start_time and end_time"""

        in_data = session.query(func.count(ScootPercentChange.detector_id).label("sensors_with_readings"),
                                ScootPercentChange.measurement_start_utc).filter(ScootPercentChange.baseline_period == baseline)

        if start_time:
            in_data = in_data.filter(ScootPercentChange.measurement_start_utc >= start_time)
        if end_time:
            in_data = in_data.filter(ScootPercentChange.measurement_start_utc < end_time)
        ...
        in_data_cte = in_data.group_by(ScootPercentChange.measurement_start_utc).cte("in_data")

        if end_time:
            expected_date_times = session.query(func.generate_series(
                start_time, end_time, ONE_DAY_INTERVAL).label("measurement_start_utc")).subquery()
        else:
            expected_date_times = session.query(func.generate_series(
                start_time, func.date_trunc("day", func.now()) - ONE_DAY_INTERVAL, ONE_DAY_INTERVAL).label("measurement_start_utc")).subquery()

        available_data_q = session.query(expected_date_times.c.measurement_start_utc,
                                         ...
                                         ).select_entity_from(expected_date_times).join(in_data_cte,
                                                                                        in_data_cte.c.measurement_start_utc == expected_date_times.c.measurement_start_utc,
                                                                                        isouter=True)

        if only_missing:
            return available_data_q.filter(in_data_cte.c.sensors_with_readings.is_(None))
        else:
...
class ScootHourlyAvailability(APIQueryMixin):
    ...
    def query(self, session, start_time, end_time=None, only_missing=False):
        ...
        in_data = session.query(func.count(ScootReading.detector_id).label("sensors_with_readings"),
                                ScootReading.measurement_start_utc).filter(ScootReading.measurement_start_utc >= start_time)
        if end_time:
            in_data = in_data.filter(ScootReading.measurement_start_utc < end_time)

        in_data_cte = in_data.group_by(ScootReading.measurement_start_utc).cte("in_data")

        if end_time:
            expected_date_times = session.query(func.generate_series(
                start_time, end_time,  ONE_HOUR_INTERVAL).label("measurement_start_utc")).subquery()
        else:
            expected_date_times = session.query(func.generate_series(
                start_time, func.now(), ONE_HOUR_INTERVAL).label("measurement_start_utc")).subquery()
